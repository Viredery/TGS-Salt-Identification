...
def criterion(logit_fuse, logit_pixel, logit_image, truth_pixel, truth_image):
    ...
    # non-empty loss
    loss_pixel = dice_bce_loss_per_image()(logit_pixel.sigmoid(), truth_pixel)
    loss_pixel = loss_pixel * truth_image
    loss_pixel = loss_pixel.sum() / (truth_image.sum() + 1e-12)

    # all images loss
    loss_fuse = dice_bce_loss()(logit_fuse.sigmoid(), truth_pixel)
...
for cv_num, (train_idx, val_idx) in enumerate(cv.split(data_ids, data_class)):
    ...
    f = open('../logs/{}_{}_{}.txt'.format(NAME, cv_num, MODE), 'w+')
    f.close()
    ...
    while epoch < num_epochs:
        ...
        if eval_iou > best_iou_metric:
            ...
            torch.save(net.state_dict(), '../weights/{}_{}_{}.th'.format(NAME, cv_num, MODE))

        with open('../logs/{}_{}_{}.txt'.format(NAME, cv_num, MODE), 'a+') as f:
